...
export function InteractiveMenuDashboard() {
  ...
  const [editingProduct, setEditingProduct] = useState<ProductDraft | null>(
    () => productToDraft(initialProducts[0]),
  );
  const [newProductDraft, setNewProductDraft] = useState<ProductDraft>(() =>
    blankProductDraft(categoryItems[0]?.id ?? "general"),
  );
  ...
  const summaryItems: Array<
    SummaryItem & { actionLabel: string; onAction: () => void }
  > = [
    {
      value: String(categories.length),
      label: "Catégories",
      helper: "Toutes vos catégories",
      tone: "emerald",
      actionLabel: "Supprimer toutes les catégories",
      onAction: handleDeleteAllCategories,
    },
    {
      value: String(
        products.filter((product) => product.available && product.visible)
          .length,
      ),
      label: "Produits actifs",
      helper: "Disponibles à la vente",
      tone: "sky",
      actionLabel: "Supprimer tous les produits actifs",
      onAction: handleDeleteActiveProducts,
    },
    {
      value: String(products.filter((product) => !product.available).length),
      label: "Produit en rupture",
      helper: "Non disponible",
      tone: "rose",
      actionLabel: "Supprimer tous les produits en rupture",
      onAction: handleDeleteUnavailableProducts,
    },
    {
      value: String(products.filter((product) => product.promo).length),
      label: "Promotions actives",
      helper: "En cours",
      tone: "amber",
      actionLabel: "Retirer toutes les promotions actives",
  ...
  const filteredProducts = useMemo(() => {
    ...
    return products.filter((product) => {
      ...
      const matchesFilter =
        ...
        (activeFilter === "promo" && product.promo);
  ...
  const syncSelectedProduct = (
    ...
    preferredProductId: string | null = selectedProductId,
  ) => {
    const nextSelected =
      (preferredProductId
        ? nextProducts.find((product) => product.id === preferredProductId)
        : null) ??
      nextProducts[0] ??
      null;
  ...
  function handleDeleteAllCategories() {
    if (
      !window.confirm(
        "Voulez-vous vraiment supprimer toutes les catégories de cette maquette ? Les produits seront conservés mais repassés sans catégorie.",
      )
    )
      return;
    ...
    syncSelectedProduct(nextProducts);
    setReorderMode(false);
    showMessage("Catégories supprimées de la maquette.");
  }

  function handleDeleteActiveProducts() {
    if (
      !window.confirm(
        "Voulez-vous vraiment supprimer tous les produits actifs de cette maquette ?",
      )
    )
      return;

    const nextProducts = products.filter(
      (product) => !(product.available && product.visible),
    );
    setProducts(nextProducts);
    syncSelectedProduct(nextProducts, null);
    setRowAction(null);
    showMessage("Produits actifs supprimés de la maquette.");
  }

  function handleDeleteUnavailableProducts() {
    if (
      !window.confirm(
        "Voulez-vous vraiment supprimer tous les produits en rupture de cette maquette ?",
      )
    )
      return;
  ...
  function handleRemoveAllPromotions() {
    if (
      !window.confirm(
        "Voulez-vous vraiment retirer toutes les promotions actives de cette maquette ?",
      )
    )
      return;
  ...
  function handleMoveCategoryUp(categoryId: string) {
    setCategories((currentCategories) => {
      ...
      if (categoryIndex <= 0) return currentCategories;

      const nextCategories = [...currentCategories];
      [nextCategories[categoryIndex - 1], nextCategories[categoryIndex]] = [
        nextCategories[categoryIndex],
        nextCategories[categoryIndex - 1],
      ];
  ...
  function handleMoveCategoryDown(categoryId: string) {
    setCategories((currentCategories) => {
      ...
      if (categoryIndex === -1 || categoryIndex >= currentCategories.length - 1)
        return currentCategories;

      const nextCategories = [...currentCategories];
      [nextCategories[categoryIndex], nextCategories[categoryIndex + 1]] = [
        nextCategories[categoryIndex + 1],
        nextCategories[categoryIndex],
      ];
      return nextCategories;
  ...
  const openRowActions = useCallback(
    (productId: string, buttonRect: DOMRect) => {
      const dropdownWidth = 192;
      const dropdownHeight = 188;
      const margin = 16;
      const top =
        buttonRect.bottom + dropdownHeight + margin > window.innerHeight
          ? Math.max(margin, buttonRect.top - dropdownHeight - 8)
          : buttonRect.bottom + 8;
      const left = Math.min(
        Math.max(margin, buttonRect.right - dropdownWidth),
  ...
  const editProductFromMenu = (productId: string) =>
    selectProduct(productId, { focusPanel: true });

  const saveEditedProduct = () => {
    if (!selectedProductId || !editingProduct) return;

    const normalizedDraft = normalizeDraftPrice(
      editingProduct,
      selectedProduct?.price,
    );
    const normalizedPromoValue = formatPromotionValue(
      editingProduct.promoValue,
      editingProduct.promoType,
    );
    const parsedPrice =
      parseEuroInput(normalizedDraft.price) ?? selectedProduct?.price ?? 0;
    const savedDraft = {
      ...normalizedDraft,
      name: normalizedDraft.name.trim(),
      description: normalizedDraft.description.trim(),
      allergens: normalizedDraft.allergens
  ...
  const toggleVisibility = (productId: string) => {
    ...
    if (selectedProductId === productId) {
      setEditingProduct((currentDraft) =>
        currentDraft
          ? { ...currentDraft, visible: !currentDraft.visible }
          : currentDraft,
      );
    }
  ...
  const deleteProduct = (productId: string) => {
    ...
    if (
      !product ||
      !window.confirm(`Supprimer ${product.name} de la maquette ?`)
    )
      return;
    ...
    const nextSelected =
      remainingProducts.find((item) => item.available && item.visible) ??
      remainingProducts[0] ??
      null;
    setSelectedProductId(nextSelected?.id ?? null);
    setEditingProduct(productToDraft(nextSelected));
  ...
  const addProduct = () => {
    ...
    if (!newProductDraft.name.trim() || parsedPrice === null) {
      showMessage(
        "Renseignez au minimum le nom et un prix valide pour le produit.",
      );
      return;
    }
  ...
  return (
    <>
      ...
      >
        <button
          className="rounded-2xl border border-slate-200 bg-white px-4 py-3 text-sm font-black text-slate-700 shadow-sm transition hover:border-emerald-200 hover:bg-emerald-50"
        ...
        <button
          className="rounded-2xl bg-emerald-700 px-4 py-3 text-sm font-black text-white shadow-lg shadow-emerald-900/15 transition hover:bg-emerald-800"
      ...
      <main className="flex-1 space-y-7 p-5 lg:p-8">
        <RestaurantSelectorCard
          onClick={() =>
            showMessage(
              "Le changement de restaurant est désactivé dans cette maquette locale.",
            )
          }
        />
          ...
          <div className="space-y-7">
            <div ref={editPanelRef}>
              <ProductEditPanel
                ...
                onCancel={cancelEditedProduct}
                onDraftChange={setEditingProduct}
                onNormalizePrice={() =>
                  setEditingProduct((currentDraft) =>
                    currentDraft
                      ? normalizeDraftPrice(
            ...
            <PublicMenuPreview
              ...
              selectedCategoryId={selectedCategoryId}
              onCartClick={() =>
                showMessage("Le panier public est simulé dans cette maquette.")
              }
            />
          </div>
        </section>
          ...
          <ProductForm
            ...
            onNormalizePrice={() =>
              setNewProductDraft((currentDraft) =>
                normalizeDraftPrice(currentDraft, undefined),
              )
            }
          />
          <ModalActions
      ...
      {isAddCategoryOpen ? (
        ...
        >
          <label className="block">
            ...
            <input
              className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 outline-none focus:border-emerald-300 focus:bg-white focus:ring-4 focus:ring-emerald-100"
...
function Toast({ message, onClose }: { message: string; onClose: () => void }) {
  return (
    <div
      className="fixed right-4 top-4 z-[120] flex w-[min(calc(100vw-2rem),28rem)] items-start gap-3 rounded-3xl border border-emerald-200 bg-white px-5 py-4 text-sm font-black text-emerald-900 shadow-2xl shadow-slate-950/20 ring-1 ring-emerald-100 sm:right-6 sm:top-6"
      role="status"
    >
      <span className="mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-emerald-700 text-xs text-white">
        ✓
      </span>
      <span className="min-w-0 flex-1 leading-6">{message}</span>
      <button
        aria-label="Fermer la notification"
        className="rounded-full px-2 text-lg leading-none text-slate-400 transition hover:bg-slate-100 hover:text-slate-700"
        onClick={onClose}
        type="button"
      >
        ×
  ...
  return (
    <div className="space-y-4">
      ...
      <label className="block">
        ...
        <select
          className="mt-2 w-full rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 outline-none"
...
function FormField({
  ...
  return (
    <label className="block">
      ...
      {multiline ? (
        <textarea
          className="mt-2 min-h-24 w-full resize-none rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm font-semibold text-slate-700 outline-none focus:border-emerald-300 focus:bg-white focus:ring-4 focus:ring-emerald-100"
          onBlur={onBlur}
...
function Modal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-950/40 p-4 backdrop-blur-sm">
      <section
        className={`max-h-[90vh] w-full overflow-y-auto rounded-[2rem] border border-slate-200 bg-white p-5 shadow-2xl shadow-slate-950/20 ${wide ? "max-w-xl" : "max-w-lg"}`}
      >
...
function ModalActions({
  ...
  return (
    <div className="mt-6 grid grid-cols-2 gap-3">
      ...
      <button
        className="rounded-2xl bg-emerald-700 px-4 py-3 text-sm font-black text-white shadow-lg shadow-emerald-900/15"
        onClick={onSave}
        type="button"
      >
        {saveLabel}
