...
export function PublicMenuPreview({ activeCategoryId = "plats", categories = [], products = [], selectedProduct }: PublicMenuPreviewProps) {
  const previewCategories = categories.length > 0 ? categories : [
  ...
  const visibleProducts = products.filter((product) => product.visible && product.availability === "Disponible").slice(0, 3);
  const previewProducts = selectedProduct && selectedProduct.visible ? [selectedProduct, ...visibleProducts.filter((product) => product.id !== selectedProduct.id)].slice(0, 3) : visibleProducts;
            ...
            <div className="space-y-4 bg-slate-50 px-4 py-5">
              <div className="flex gap-2 overflow-hidden">
                {previewCategories.map((category) => (
                  <span className={`shrink-0 rounded-full px-3 py-2 text-xs font-black ${category.id === activeCategoryId ? "bg-emerald-700 text-white" : "bg-white text-slate-500"}`} key={category.id}>{category.name}</span>
                ))}
              </div>

              {previewProducts.length > 0 ? previewProducts.map((product) => (
                <PreviewRow key={product.id} product={product} />
              )) : <p className="rounded-3xl bg-white p-4 text-sm font-bold text-slate-500">Aucun produit visible dans l’aperçu.</p>}
...
function PreviewRow({ product }: { product: ProductItem }) {
  return (
    <article className="flex gap-3 rounded-3xl bg-white p-3 shadow-sm shadow-slate-200/70">
      <span className={`h-16 w-16 shrink-0 rounded-2xl bg-gradient-to-br ${product.imageTone}`} />
      <span className="min-w-0 flex-1">
        <span className="flex items-start justify-between gap-2">
          <span className="font-black text-slate-950">{product.name}</span>
          {product.promo === "Promo" ? <span className="rounded-full bg-amber-100 px-2 py-0.5 text-[10px] font-black text-amber-700">{product.promotionValue || "Promo"}</span> : null}
        </span>
