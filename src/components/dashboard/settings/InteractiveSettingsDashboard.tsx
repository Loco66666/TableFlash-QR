...
const qrStyles: RestaurantSettings["qrStyle"][] = [
  "Classique",
  "Premium",
  "Minimal",
];
...
function TimeField({
  ...
  return (
    <label className="block min-w-[92px]">
      ...
      <input
        ...
        value={value}
        onChange={(event) => onChange(event.target.value)}
        className="mt-2 h-12 w-full min-w-[92px] rounded-2xl border border-slate-200 bg-white px-3 text-sm font-black text-slate-950 outline-none transition focus:border-emerald-500 focus:ring-4 focus:ring-emerald-100"
      />
    </label>
  );
}
      ...
      <div className="mt-2 flex max-w-full flex-wrap gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-1.5">
        {options.map((option) => (
          <button
            ...
            className={`min-h-11 flex-1 basis-[120px] rounded-xl px-4 py-2.5 text-sm font-black transition ${value === option ? "bg-emerald-700 text-white shadow-lg shadow-emerald-900/20" : "text-slate-600 hover:bg-white hover:text-emerald-700"}`}
          >
    ...
    >
      <section className="min-w-0 rounded-[2rem] border border-emerald-100 bg-white p-5 shadow-xl shadow-emerald-950/10">
        ...
        <p className="mt-2 break-words text-sm leading-6 text-slate-500">
          Une synthèse mobile, lisible et fidèle aux réglages actifs.
        </p>
  ...
  const renderActiveSection = () => {
    if (activeSection === "establishment") {
      return (
        <SettingsCard
          ...
          title="Établissement"
          description="Centralisez les informations qui alimentent les liens publics, les QR et l’expérience client."
        >
          <div className="grid min-w-0 gap-4 lg:grid-cols-2">
            ...
            <div className="min-w-0">
              ...
              <p className="mt-2 break-words text-xs font-semibold leading-5 text-slate-500">
                Ce slug sera utilisé pour les liens publics et les QR.
              </p>
            </div>
            <TextField
              label="Site web"
          ...
          <div className="mt-5 flex min-w-0 flex-col gap-3 rounded-2xl border border-emerald-100 bg-emerald-50 p-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <button
              ...
              className="min-h-11 shrink-0 rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white transition hover:bg-slate-800"
            >
              Générer le slug
            </button>
          </div>
    ...
    if (activeSection === "service") {
      return (
        <SettingsCard
          ...
          title="Service"
          description="Indiquez si le restaurant prend des commandes et gardez un planning hebdomadaire lisible."
        >
          ...
          <div className="mt-6 min-w-0 space-y-3">
            {settings.hours.map((hour) => (
              <div
                key={hour.day}
                className={`grid min-w-0 gap-4 rounded-[1.5rem] border border-slate-200 p-4 lg:grid-cols-[150px_140px_minmax(0,1fr)_minmax(0,1fr)] lg:items-center ${hour.open ? "bg-white" : "bg-slate-50/80"}`}
              >
                <div className="min-w-0">
                  <p
    ...
    if (activeSection === "orders") {
      return (
        <SettingsCard
          eyebrow="Opérations"
          title="Commandes"
          description="Transformez les réglages de commande en consignes compréhensibles pour l’équipe et les clients."
        >
          <div className="grid min-w-0 gap-4 xl:grid-cols-2">
            <div className="min-w-0 rounded-[1.5rem] border border-slate-200 bg-slate-50/70 p-5 xl:col-span-2">
              <p className="break-words text-sm font-black text-slate-950">
                Règlement
              </p>
              <p className="mt-2 break-words text-sm font-semibold leading-6 text-slate-500">
                TableFlash ne déclenche aucun paiement en ligne dans ce MVP. Le
                règlement reste physique.
              </p>
              <div className="mt-4 space-y-4">
                ...
                />
                <ToggleRow
                  label="Exiger le règlement avant préparation"
                  helper="Clarifie le passage en caisse ou auprès du serveur."
            ...
            <div className="min-w-0 rounded-[1.5rem] border border-slate-200 bg-white p-5">
              <p className="break-words text-sm font-black text-slate-950">
                Workflow commande
              </p>
              <div className="mt-4 space-y-3">
                <ToggleRow
                  label="Accepter automatiquement les commandes"
                  helper="Désactivé par défaut pour conserver une validation manuelle."
                ...
                <ToggleRow
                  ...
                  helper="Allergies, cuisson ou demandes spéciales restent visibles."
            ...
            <div className="min-w-0 rounded-[1.5rem] border border-slate-200 bg-white p-5">
              ...
              <div className="mt-4 space-y-3">
                <ToggleRow
                  ...
                  helper="Le client sait si la commande est validée, en préparation ou prête."
    ...
    if (activeSection === "qr") {
      return (
        <SettingsCard
          ...
          title="QR"
          description="Contrôlez la fiche QR imprimée et les zones utilisées pour organiser les emplacements."
        >
          <div className="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,1fr)_320px]">
            <div className="min-w-0 space-y-5">
              <SegmentedControl
                ...
                options={qrStyles}
                onChange={(value) => updateSetting("qrStyle", value)}
              />
              <TextAreaField
            ...
            <div className="min-w-0 rounded-[1.75rem] border border-emerald-100 bg-emerald-50 p-4">
              <div className="min-w-0 rounded-[1.25rem] bg-white p-4 text-center shadow-sm">
                ...
                <span className="mt-3 inline-flex whitespace-nowrap rounded-full bg-emerald-50 px-3 py-1 text-xs font-black text-emerald-700">
                  {settings.qrStyle}
                </span>
              </div>
            </div>
            ...
            <div className="flex min-w-0 flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="min-w-0">
                ...
                <p className="mt-1 break-words text-sm font-semibold leading-6 text-slate-500">
                  Salle, terrasse, comptoir ou toute zone qui reçoit des QR.
                </p>
              </div>
              <button
                type="button"
                onClick={addZone}
                className="min-h-11 shrink-0 rounded-full bg-emerald-700 px-5 py-3 text-sm font-black text-white shadow-lg shadow-emerald-900/20 transition hover:bg-emerald-800"
              >
                Ajouter une zone
              </button>
            </div>
              ...
              {settings.zones.map((zone) => (
                ...
                >
                  ...
                  <button
                    ...
                    className="min-h-12 rounded-full border border-rose-200 bg-white px-4 py-3 text-sm font-black text-rose-700 transition hover:bg-rose-50 disabled:cursor-not-allowed disabled:opacity-40"
                  >
                    Supprimer
                  </button>
                </div>
    ...
    if (activeSection === "reviews") {
      return (
        <SettingsCard
          eyebrow="Réputation"
          title="Avis"
          description="Les avis sont proposés après le repas, lorsque la commande est terminée."
        >
          <div className="grid min-w-0 gap-5 lg:grid-cols-[minmax(0,1fr)_320px]">
            <div className="min-w-0 space-y-4">
              ...
              <ToggleRow
                ...
                helper="Aucune publication automatique Google n’est déclenchée."
            ...
            <div className="min-w-0 space-y-3 rounded-2xl bg-slate-50 p-4">
              <p className="break-words text-sm font-bold leading-6 text-slate-600">
                Le lien peut être proposé au client après son avis, sans
                promesse de publication automatique.
              </p>
              <button
                type="button"
                onClick={() =>
                  showToast("Lien Google Avis copié dans la maquette.")
                }
                className="min-h-12 w-full rounded-full bg-slate-950 px-5 py-3 text-sm font-black text-white transition hover:bg-slate-800"
              >
                Copier le lien
              </button>
              <button
                type="button"
                onClick={() =>
                  showToast("Ouverture Google Avis simulée dans la maquette.")
                }
                className="min-h-12 w-full rounded-full border border-slate-200 bg-white px-5 py-3 text-sm font-black text-slate-700 transition hover:border-emerald-200 hover:bg-emerald-50"
              >
                Tester le lien
              </button>
            </div>
    ...
    return (
      <SettingsCard
        ...
        title="Apparence"
        description="Choisissez une direction visuelle premium pour l’expérience client, sans changer le thème global de l’application."
      >
        <div className="grid min-w-0 gap-3 sm:grid-cols-2">
          {primaryColors.map((color) => (
            ...
            >
              <span className="block whitespace-nowrap text-lg">{color}</span>
              <span className="mt-6 inline-flex rounded-full bg-white/20 px-3 py-1 text-xs">
  ...
  return (
    <>
      ...
      <main className="flex-1 overflow-x-hidden bg-slate-50/70 p-4 sm:p-5 lg:p-8">
        <div className="mx-auto grid min-w-0 max-w-[1500px] gap-6 2xl:grid-cols-[minmax(0,1fr)_360px]">
