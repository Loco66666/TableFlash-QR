...
export function PeakHoursChart({ hours }: { hours: PeakHour[] }) {
  ...
  return (
    <article className="min-w-0 max-w-full overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white p-6 shadow-sm shadow-slate-200/60">
      <h2 className="break-words text-2xl font-black text-slate-950">Heures de pointe</h2>
      <div className="mt-6 space-y-4">
        {hours.map((hour) => (
          <div key={hour.hour} className="grid min-w-0 grid-cols-[3rem_1fr_4rem] items-center gap-3">
