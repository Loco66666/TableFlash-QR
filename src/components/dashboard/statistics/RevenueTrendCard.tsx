import type { ActivityBucket } from "./statisticsActivityBuckets";
...
export function RevenueTrendCard({ points }: { points: ActivityBucket[] }) {
  const maxOrders = Math.max(...points.map((point) => point.orders), 1);
  const peak = points.reduce<ActivityBucket | null>((bestPoint, point) => {
    ...
    <article className="min-w-0 max-w-full overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white p-5 sm:p-6 shadow-sm shadow-slate-200/60">
      <div className="flex min-w-0 flex-col gap-2 sm:flex-row sm:items-start sm:justify-between">
        <div className="min-w-0">
          <p className="break-words text-sm font-black uppercase tracking-[0.08em] text-emerald-700">
          ...
          <p className="mt-2 break-words text-sm font-semibold leading-6 text-slate-500">
            Repérez les pics de commandes et les créneaux les plus chargés.
          </p>
        </div>
        {peak ? (
          <div className="max-w-full break-words rounded-2xl bg-emerald-50 px-4 py-3 text-sm font-black text-emerald-800">
            Pic d’activité : {peak.label} — {peak.orders} commandes
          </div>
        ) : null}
      </div>
      ...
      <div className="mt-5 grid min-w-0 max-w-full grid-cols-2 gap-3 md:grid-cols-3 xl:grid-cols-4 2xl:grid-cols-6">
        {points.map((point) => {
          const height = Math.max(
            18,
            Math.round((point.orders / maxOrders) * 78),
          );
            ...
            >
              <div className="flex h-24 w-full items-end justify-center rounded-3xl bg-slate-50 px-3 py-3">
                <div
                  ...
                  style={{ height }}
                  aria-label={`${point.orders} commandes à ${point.label}`}
                />
              </div>
