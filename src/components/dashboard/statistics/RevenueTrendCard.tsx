  ...
  return (
    <article className="min-w-0 max-w-full overflow-hidden rounded-[2rem] border border-slate-200/80 bg-white p-5 sm:p-6 shadow-sm shadow-slate-200/60">
      ...
      <div className="mt-6 grid min-w-0 max-w-full grid-cols-1 gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {points.map((point) => {
          const height = Math.max(24, Math.round((point.orders / maxOrders) * 118));
          const isPeak = point.hour === peak.hour;

          return (
            <div key={point.hour} className="flex min-w-0 flex-col items-center justify-end gap-3">
              <div className="flex h-32 w-full items-end justify-center rounded-3xl bg-slate-50 px-3 py-4 sm:h-36">
                <div
              ...
              <div className="w-full rounded-2xl bg-white/80 px-3 py-2.5 text-center shadow-sm shadow-slate-100/80 ring-1 ring-slate-100">
