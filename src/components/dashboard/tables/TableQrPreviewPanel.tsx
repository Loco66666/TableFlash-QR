...
export function TableQrPreviewPanel({ onCopy, onDownload, onEdit, onPrint, onToggleActive, table }: TableQrPreviewPanelProps) {
  if (!table) {
    return (
      <aside className="rounded-[1.5rem] border border-dashed border-slate-300 bg-white/80 p-4 text-center shadow-sm lg:sticky lg:top-6 lg:self-start">
  ...
  return (
    <aside className="rounded-[1.5rem] border border-slate-200/80 bg-white/95 p-4 shadow-lg shadow-emerald-950/[0.04] lg:sticky lg:top-6 lg:self-start">
...
function SecondaryActionButton({ children, onClick }: { children: React.ReactNode; onClick: () => void }) {
  return (
    <button className="rounded-2xl border border-slate-200 bg-white px-4 py-2.5 text-sm font-black text-slate-700 transition hover:border-emerald-200 hover:bg-emerald-50 hover:text-emerald-800" onClick={onClick} type="button">
      {children}
    </button>
  );
}
