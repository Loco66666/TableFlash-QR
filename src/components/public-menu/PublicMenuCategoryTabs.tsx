...
export function PublicMenuCategoryTabs({
  ...
  return (
    <nav
      className="sticky top-0 z-20 -mx-4 max-w-none overflow-hidden border-y border-emerald-950/5 bg-[#F8FAF7]/95 py-3 backdrop-blur-xl"
      aria-label="Catégories du menu"
    >
      <div className="overflow-x-auto overscroll-x-contain px-4 pb-1 [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        <div className="flex min-w-max flex-nowrap gap-2 pr-4">
          {categories.map((category) => {
            ...
            return (
              <button
                ...
                className={`flex min-h-10 shrink-0 scroll-mx-4 items-center gap-2 whitespace-nowrap rounded-full border px-4 text-sm font-extrabold transition active:scale-[0.98] ${
                  isActive
