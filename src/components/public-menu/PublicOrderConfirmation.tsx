...
const customerStatusContent: Record<LocalOrderStatus, CustomerStatusContent> = {
  Nouvelle: {
    ...
    message: "Votre demande a été envoyée au restaurant.",
    nextStep: "L’équipe vérifie votre commande avant validation.",
  },
  Acceptée: {
    title: "Validation confirmée",
    message: "L’équipe a confirmé votre commande.",
    nextStep:
      "Le règlement s’effectue sur place avant le lancement en préparation.",
  },
  "À payer": {
    title: "Règlement attendu",
    message: "Votre commande est prête à être lancée côté restaurant.",
    nextStep:
      "Réglez à la caisse ou auprès du serveur pour démarrer la préparation.",
  },
  Payée: {
    title: "Règlement confirmé",
    message: "Votre règlement a été pris en compte.",
    nextStep: "La préparation peut démarrer dès que l’équipe est prête.",
  },
  "En préparation": {
    title: "En préparation",
    message: "L’équipe prépare actuellement votre commande.",
    nextStep: "Nous vous préviendrons dès qu’elle sera prête à être servie.",
  },
  Prête: {
    title: "Prête à servir",
    message: "Votre commande est prête côté restaurant.",
    nextStep: "Un membre de l’équipe va la déposer à votre table.",
  },
  Servie: {
    ...
    nextStep:
      "Vous pouvez maintenant partager votre avis si vous le souhaitez.",
  },
  Refusée: {
...
function getPreparationLabel(status: LocalOrderStatus) {
  if (["En préparation", "Prête", "Servie"].includes(status)) {
    return "Cuisine lancée";
  }
...
export function PublicOrderConfirmation({
  ...
  const preparationLabel = getPreparationLabel(currentStatus);
      ...
      <div className="max-h-[92vh] w-full max-w-[460px] overflow-y-auto rounded-t-[2.25rem] bg-white p-5 shadow-2xl md:rounded-[2.25rem]">
        <div className="rounded-[1.75rem] bg-gradient-to-br from-emerald-50 to-white p-5 text-center ring-1 ring-emerald-100">
          ...
          <h2
            ...
            className="mt-4 text-3xl font-black tracking-[-0.05em] text-slate-950"
          >
            Commande confirmée
          </h2>
          <p className="mx-auto mt-3 max-w-[20rem] text-sm leading-6 text-slate-600">
            Nous l’avons bien reçue. Retrouvez le récapitulatif et le suivi
            juste en dessous.
          </p>
        </div>
            ...
            {order.items.map((item) => (
              ...
              >
                <div>
                  ...
                  {item.note ? (
                    <p className="mt-1 text-slate-500">Note : {item.note}</p>
                  ) : null}
                </div>
        ...
        <section
          className="mt-4 rounded-[1.75rem] border border-emerald-100 bg-gradient-to-br from-emerald-50 via-white to-white p-4 shadow-sm shadow-emerald-950/5"
              ...
              {trackingSteps.map((step, index) => {
                ...
                return (
                  <li key={step.label} className="min-w-0">
                    ...
                    <p
                      className={`mt-2 text-center text-[0.72rem] font-black leading-4 ${isHighlighted ? "text-slate-900" : "text-slate-400"}`}
                    >
                      {step.label}
                    </p>
                  </li>
              ...
              {hasSubmittedReview ? (
                <>
                  ...
                  <p className="mt-2 text-sm leading-6 text-emerald-50">
                    Merci, votre avis a bien été transmis.
                  </p>
                </>
              ) : (
                <>
                  ...
                  <p className="mt-2 text-sm leading-6 text-emerald-50">
                    Votre commande est terminée. Si vous le souhaitez, partagez
                    votre expérience en quelques secondes.
                  </p>
                  <button
                    type="button"
        ...
        {paymentIsPending ? (
          <div className="mt-4 rounded-3xl border border-amber-100 bg-amber-50/80 p-4 text-sm leading-6 text-amber-900">
            <p className="font-black">Règlement sur place</p>
            <p className="mt-1">{paymentNote}</p>
            <p className="mt-1 font-bold">
              Présentez simplement le numéro {order.orderNumber}.
            </p>
          </div>
        ) : null}
